...
module.exports = (authService, userService, emailService) => {
    ...
    userService.getByEmail(req.body.email)
      .then(existingUser => {
        ...
        return userService.registerUser(req.body)
          .then(user => {
            user._doc.token = authService.getToken(user)
            return res.status(HttpStatus.CREATED).json(new Hypermedia(req).setResponse(user, next))
              .then(() => {
  ...
  /**
   * TODO:
   * replace res.render and return json
   */
  ...
  const resetPassword = (req, res, next) =>
    userService.getByEmail(req.body.email)
      ...
      .then(data => res.status(HttpStatus.OK).json({data: data}))
      .catch(next)
