...
describe('User schema validation tests', () => {
    ...
    describe('Given a valid password ', () => {
        ...
        it('will pass the validation', sinon.test(function (done) {
            user.isValidPassword(passVar).then(result => {
                expect(result).to.equal(true)
                done()
            }).catch(err => done(err))
        }))

        it('will failed validation', sinon.test(function (done) {
            user.isValidPassword('other password').then(result => {
                expect(result).to.equal(false)
                done()
            }).catch(err => done(err))
        }))

        it('will thrown an error', sinon.test(function (done) {
            this.stub(bcrypt, 'compare').yields(new Error('Error Message'), null);
            user.isValidPassword('other password').catch(err => {
